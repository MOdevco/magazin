...
export default function Monitor() {
  ...
  useEffect(() => {
    axios.get(`${api}api/monitors/get-data`, {
      headers: {
  ...
  return (
    <Box>
      ...
      <Box mt={20} className='keybored' w='90%' m='auto'>
        ...
        <Text fontSize='32px' fontWeight='700' mt='40px' color='#333'>Monitorlar</Text>
          ...
          <Box display='flex' alignItems='center' justifyContent='space-between'flexWrap='wrap' >
            ...
            {data.slice(0, 4).map((item, i) => (
              <Box key={i} display='flex' flexDirection='column' gap={4} width='302px' padding='10px' mb='2px' boxShadow='rgba(50, 50, 93, 0.25) 0px 6px 12px -2px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px;'>
               ...
               <Link to={`/product/monitors/${item._id}`}><Img src={item.image} height='300px' objectFit='cover' position='relative' /></Link>
                <Box textAlign='start'>
